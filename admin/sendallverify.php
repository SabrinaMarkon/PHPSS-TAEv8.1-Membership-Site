...
$email = $_POST['email'];
	...
	if(!$mail2->Send()) {
	  ...
	  echo "Message sent to " . $each['userid'] . " at " . $each['contact_email'] . "<br>";
	}
	}
	else {
	...
	@mail($each['contact_email'], "[".$sitename."] Email Verification", wordwrap(stripslashes($message)),"From: $from_address", "-f $return_path");
	echo "Message sent to " . $each['userid'] . " at " . $each['contact_email'] . "<br>";
	}
