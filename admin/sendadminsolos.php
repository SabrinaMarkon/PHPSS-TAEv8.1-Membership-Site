...
//now we've got all we need, so blast the ad out!
    ...
    $Message .= "You can opt out of receiving emails only by deleting your account, click here to delete your account.<br><br><a href=\"$domain/delete.php?userid=$useridq&code=$passwordq\">$domain/delete.php?userid=$useridq&code=$passwordq</a>.<br><br>";
    ...
    $mail2->AddAddress($email, $firstnameq);
	if(!$mail2->Send()) {
	  echo "Mailer Error: " . $mail2->ErrorInfo . "<br>";
	} else {
	  echo "Message sent to " . $useridq . " at " . $email . "<br>";
	}
	}
	else {
