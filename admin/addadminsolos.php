...
if (isset($_POST['action'])) {
   $action = mysql_real_escape_string($_POST['action']);
	    ...
	    if ($solostoadd == 1) {
				...
				$query5 = "SELECT * FROM adminsolos where added=1 and sent=0";
...
<?
    if ($action == "send") {
		...
		if ($sendingemail == 'other' && empty($senderemail)) {
       		?><p>No reply email entered. Click <a href=addadminsolos.php>here</a> to go back<p> <?
       		include "../footer.php";
       		exit;
		}
    	...
    	$query = "update adminsolos set replyemail='$replyemail', subject='$subject', adbody='$adbody', added=1, approved=1, userid='admin', sent=0, url='$url',date='".time()."' where id=".$id;
	...
	//Solo available
	$query = "SELECT * FROM adminsolos where added=0 limit 1";
	$result = mysql_query ($query)
		or die ("Query failed");
