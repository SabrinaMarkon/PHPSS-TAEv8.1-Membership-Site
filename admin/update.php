                ...
                echo $userid." has had ".$propointsmonthly." points added to their account.<br>";
 	            ...
 	            elseif ($memtype=="JV Member") {
 	                $commission=$proprice*$jvcommission/100;
                                ...
                                $commission=$proprice*$superjvcommissions/100;
                            }
            ...
            if ($subscribed == $onemonthago) {
               ...
               echo $userid." has had ".$superjvpointsmonthly." points added to their account.<br>";
	            ...
	            $memtype = $line["memtype"];
                                 ...
                                 if ($memtype=="PRO") {
 	                $commission=$superjvprice*$prosupercom/100;
 	            }
 	            elseif ($memtype=="JV Member") {
 	                $commission=$superjvprice*$jvsupercom/100;
 	            } elseif ($memtype=="SUPER JV") {
                                $commission=$superjvprice*$superjv2supercom/100;
                            }
 ...
 while($auto = mysql_fetch_array($sql)) {
	...
	include "../SMTP.php";
     ...
     while ($line = mysql_fetch_array($sql2)) {
 		...
 		$Message = "*********************<br>As a valued member of $sitename you have agreed to receive\ncontact from us. If you no longer want our messages, follow the instructions at the bottom.<br>*********************<br><br>".$Message."<br><br>*********************<br>You are receiving this as you are a member of $sitename. You can opt out of receiving emails only by deleting your account, click here to delete your account:<br><br><href=\"$domain/delete.php?userid=$userid&code=$password\">$domain/delete.php?userid=$userid&code=$password</a><br><br>$sitename<br>$adminaddress<br>*********************<br><br>";
	...
	if (($replyemail == $bounceemail || $replyemail == "")) {
    ...
    $mail2->AddAddress($contactemail, $firstname);
	if(!$mail2->Send()) {
	...
	@mail($contactemail, $Subject, wordwrap(stripslashes($Message)),$headers, "-f$return_path");
	echo "Message sent to " . $userid . " at " . $contactemail . "<br>";
	}
 	}
