...
if($jvsignup) {
	...
	include "SMTP.php";
 	...
 	$message = "Dear ".$new_fullname." ".$new_lastname.",\n\nThank you for signing up for ".$sitename.".\nYour account details are below:\n\n"
	    ...
	    @mail($new_contact, "Welcome to ". $sitename, wordwrap(stripslashes($message)),$headers, "-f$return_path");
		}
    include "banners.php";
...
if ($adminnotice=="1") {
		...
		$headern .= "From: $sitename<$return_path>\n";
		$headern .= "Reply-To: <$return_path>\n";
		$headern .= "Bcc: <$return_path>\n";
		$headern .= "X-Sender: <$return_path>\n";
		$headern .= "X-Mailer: PHP5\n";
		$headern .= "X-Priority: 3\n";
		$headern .= "Return-Path: <$return_path>\n";
		@mail($adminemail, $subject, $body, $headern, "-f$return_path");
		}
