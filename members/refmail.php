		...
		if(@mysql_num_rows($scan)) {
			...
			if($_POST['message'] AND $_POST['subject']) {
				...
				while($each = mysql_fetch_array($sql)) {
					...
					$message .= "<br><br>---------------------------------------------------------<br>You are receiving this as you are a member of $sitename. You can opt out of receiving emails only by deleting your account, click here to delete your account.<br><br>$domain/delete.php?userid=".$each['userid']."&code=".$each['pword']."<br><br>$sitename<br>$adminaddress";
				
				$return_path = "user-".$each['userid']."-".$bounceemail;
				if ($mailermethod == "smtp") {
				...
				$mail->AddAddress($each['contact_email']);
				$mail->Send();
				}
				else {
				...
				@mail($each['contact_email'], $subject, $message, $headers, "-f $return_path");
				}
