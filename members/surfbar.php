...
elseif ($autosurf === 'yes' && $_POST['y']) {
	...
	<center>
		...
		if ($surfratiocounter >= $surfratio)
		{
			echo "<br>You have earned $earn surf credits. Redirecting to next surf url...";
		} else {
			echo "<br>Redirecting to next surf url...";
		}
		if($_SESSION['currentsurfpage_status'] == 'pending' && $_SESSION['currentsurfpage_id'] != $surf_last_id){
			$_SESSION['currentsurfpage_status'] = 'success';
			add_adclick('surfpages');
			...
			echo "<br>You have viewed all the surf pages today. Redirecting to next surf url...";
		}
		?>
	</center>
	<script>
	...
	<center>
		...
		<form name="autosurfform" id="autosurfform" style="display: inline;" method="post">
				...
				if ((0 <= 100) || (0 > 0)) {
					...
					if(counter > 0) {
						...
						if (autosurf == 'yes') {
							...
							document.getElementById('autosurfform').submit();
						} else {
