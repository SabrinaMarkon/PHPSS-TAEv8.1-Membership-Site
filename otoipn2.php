...
print "VERIFIED";
	...
	$subscriptionq = "insert into subscriptions (userid,transaction,product,paymentdate,amountreceived,status) values ('$userid','$subscr_id','OTO - Paypal Subscription','$subscr_date','$payment_amount','Paid')";
...
$transaction = $txn_id;
	...
	if ($refrows > 0) {
		...
		if ($refmemtype == "PRO") {
			$percent = $otocommissionpro;
			} elseif ($refmemtype == "JV Member") {
			$percent = $otocommissionjv;
			} elseif ($refmemtype == "SUPER JV") {
			$percent = $otocommissionsjv;
			}
			...
			$refmessage = "A member of your downline, $userid, just purchased your OTO upgrade for " . $payment_amount . "\n"
